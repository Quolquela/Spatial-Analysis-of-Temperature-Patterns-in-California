...
sayed = as.data.frame(data[ , c("AVG")])
...
#using t map but problem in variance map
library(tmap)
...
library(phylin)
Long <- seq(from=-125,to=-114,by=0.03)
Lat <- seq(from=32,to=43,by=0.03)
...
meuse$coefabsence<-results$absence

LondonWards$coefcars<-results$cars
